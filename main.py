...
# use absolute paths for Render deployment compatibility
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# load NLP model and vectorizer
model_path = os.path.join(BASE_DIR, 'nlp_model.pkl')
vectorizer_path = os.path.join(BASE_DIR, 'tranform.pkl')
...
def recommend():
    try:
        ...
        if response.status_code == 200:
            ...
            print("IMDB blocked or failed, using fallback reviews.")
            reviews_list = ["Could not fetch real reviews from IMDB.", "Please check back later."]
            reviews_status = ["Good", "Bad"]
